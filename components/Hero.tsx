          ...
          {data?.map((item: TBanner) => (
            <div key={item._id} className={`flex sm:px-6 px-2 md:px-14 lg:px-20  items-center  py-8 md:py-14 md:flex-row flex-col flex-wrap justify-around  relative gap-8 md:gap-2 `}>
                ...
                <div className="flex gap-2 items-center  mt-2">
                  <Link href={`/product/${item.buttonLink}`}>
                    <button
                      ...
                      className="py-2 px-4 hover:bg-white transition duration-300 hover:drop-shadow-lg flex gap-2 items-center rounded-md drop-shadow-md bg-white/80 text-emerald-400"
                    >
                      <BiDetail size={20} />
                      Lihat Detail
                    </button>
                  </Link>
                  <button type="button" className="py-2 px-4 flex transition duration-300 hover:bg-primary hover:drop-shadow-lg rounded-md  items-center gap-2 drop-shadow-md bg-primary/80">
                    <SlBasket size={20} />
                    <p>Add to Cart</p>
                  </button>
                </div>
              ...
              <div className="flex flex-col w-full items-start max-w-[450px] gap-2">
                <Image src={urlFor(item.image.asset._ref).width(700).url()} alt={item.alt} width={300} height={300} className="object-contain drop-shadow-2xl" />
