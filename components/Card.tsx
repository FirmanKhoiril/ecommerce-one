...
const Card = ({ name, price, normalPrice, city, details, image, discount, tags, slug: { current } }: ICardProduct) => {
  return (
    <Link href={`/${current}`} className="w-[200px] bg-white group shadow-md hover:shadow-black/10 rounded-t-lg relative h-80 flex items-start flex-col gap-2">
      ...
      <div className="px-2 py-1 flex flex-col gap-1">
        <h1 className="tracking-wide text-md">{name}</h1>
