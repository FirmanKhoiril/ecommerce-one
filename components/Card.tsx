...
const Card = ({ name, price, normalPrice, city, image, discount, slug: { current } }: ICardProduct) => {
  return (
    <div className="w-[160px] bg-white hover:scale-[1.005] min-h-[280px]  hover:border-blue-500/60 border border-transparent h-auto relative sm:static pb-2 drop-shadow-sm hover:drop-shadow-lg">
      ...
      <Button className="p-2 top-2 z-10 rounded-full drop-shadow-md hover:bg-blue-100 right-2 absolute bg-white" type="button" name="buttonAddCart" label="buttonAddCard" icon={<SlBasket size={18} />} />
    </div>
  );
};
